🚀 PHA Tracker: Reliable Asteroid Monitoring System
Project Summary
...
Key features highlighted for Engineering roles:
    ...
    Data Integrity: Secure, authenticated MongoDB setup using named volumes for persistent data.

    Data Pipeline: Separation of concerns between a dedicated data worker (ingestion) and the Express API (serving cached data).
...
RESTful API server for data retrieval.
...
High-volume, flexible data persistence layer.
...
Multi-service application defined for easy deployment.
...
Custom script for robust, scheduled data ingestion.
...
# NASA_API_KEY is required for the worker to fetch data.
...
1. Build and Start the Core Services (API & Database)

# This starts the Express API (app) and the secure MongoDB instance (mongo)
docker compose up -d app mongo

2. Execute the Data Ingestion Worker

Since the NASA API only returns 7 days of data per call, the worker script is designed to loop through all of 2022 data chunks (approx 45 calls).

# Run the worker script ONCE to populate the database with 2022 data.
# The --rm flag ensures the worker container is destroyed upon successful completion.
...
2. Security and Persistence
    ...
    Authentication: Database access requires explicit environment variables (MONGO_INIT_ROOT_USERNAME, MONGO_INIT_ROOT_PASSWORD), eliminating hardcoded credentials.
